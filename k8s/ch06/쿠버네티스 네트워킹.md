...
```shell
kubectl exec client -- nslookup mservice
```
...
```shell
kubectl exec client -- cat /etc/resolve.conf
```
...
```
![[image-188.png]]
실행 결과를 보면 node-port Pod가 worker 노드에서 수행되고 있습니다.

worker 노드의 IP 주소를 변수에 저장합니다.
```shell
...
이번에는 master 노드를 대상으로 똑같이 실습해봅니다.
```shell
