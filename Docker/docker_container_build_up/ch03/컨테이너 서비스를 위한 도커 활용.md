...
- [[#3.2.1 도커 이미지 명령어|3.2.1 도커 이미지 명령어]]
- [[#도커 이미지 태그 설정과 도커 로그인 및 로그아웃|도커 이미지 태그 설정과 도커 로그인 및 로그아웃]]
- [[#도커 로그인 방법 1 : username, password를 이용한 방법|도커 로그인 방법 1 : username, password를 이용한 방법]]
- [[#도커 로그인 방법 2 : AccessToken을 이용한 방법|도커 로그인 방법 2 : AccessToken을 이용한 방법]]
- [[#도커 이미지를 파일로 관리|도커 이미지를 파일로 관리]]
- [[#도커 이미지 삭제|도커 이미지 삭제]]
- [[#3.2.2 도커 컨테이너 명령어|3.2.2 도커 컨테이너 명령어]]
- [[#컨테이너는 프로세스이다|컨테이너는 프로세스이다]]
- [[#컨테이너 실행|컨테이너 실행]]
- [[#SQL 테스트를 위한 개발팀의 요청으로 MySQL 5.7 실행|SQL 테스트를 위한 개발팀의 요청으로 MySQL 5.7 실행]]
- [[#컨테이너 모니터링 도구 cAdvisor 컨테이너 실행|컨테이너 모니터링 도구 cAdvisor 컨테이너 실행]]
- [[#웹 서비스 실행을 위한 Nginx 컨테이너 실행|웹 서비스 실행을 위한 Nginx 컨테이너 실행]]
- [[#파이썬 프로그래밍 환경을 컨테이너로 제공|파이썬 프로그래밍 환경을 컨테이너로 제공]]
...
1. 웹 브라우저에서 `http://3.35.207.14:8080` 입력하고 접속합니다.
2. 호스트 운영체제에 8080 포트가 열려있음을 확인하고 해당 포트에 연결된 컨테이너를 찾습니다.
3. 입력한 외부 IP(3.35.207.14)와 포트번호(8080)가 컨테이너와 연결된 브리지 네트워크의 Private IP(172.17.0.2)와 포트번호(80)로 변환됩니다.
	- 즉, 3.35.207.14:8080 -> 172.17.0.2:80 으로 변환됨
4. 3단계에서 발생한 서비스가 **네트워크 주소 포트 변환(NAPT, Network Address Port Translation)** 입니다. 
